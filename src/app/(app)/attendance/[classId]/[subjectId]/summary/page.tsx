...
export default function AttendanceSummaryPage() {
    ...
    setAttendanceRecords(prevRecords => {
      const updatedRecords = prevRecords.map(record => {
        if (record.studentId === studentId) {
          const newStatus: AttendanceStatus = record.status === 'present' ? 'absent' : 'present';
          return { ...record, status: newStatus };
        }
        return record;
      });
      localStorage.setItem(`attendance-${classId}-${subjectId}`, JSON.stringify(updatedRecords));
      toast({
  ...
  }, [classId, subjectId, toast, students]); // Added students to dependency array for getStudentName
